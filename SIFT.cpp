...
namespace SIFT {
    ...
    void GetMainDirection(const cv::Mat &img, int r, int c, int radius, float sigma, int BinNum, float &omax,
        ...
        for (int i = -radius; i < radius + 1; i++) {
            int y = r + i;
            if (y <= 0 or y >= img.rows - 1) { continue; }
            for (int j = -radius; j < radius + 1; j++) {
                int x = c + j;
                if (x <= 0 or x >= img.cols - 1) { continue; }
        ...
        int length = k;
        for (auto &w: W) { w = expf(w); }

        std::vector<float> Ori(length), Mag(length);
        for (int i = 0; i < length; i++) {
            Ori[i] = atan2f(Y[i], X[i]) * 180 / CV_PI;
        ...
        for (int i = 0; i < BinNum; i++) {
            hist.push_back((temphist[i] + temphist[i + 4]) * (1.0 / 16.0) +
    ...
    void calcSIFTDescriptor(const cv::Mat &img, float c, float r, double ori, double scl, int d, int n, double *dst,
                            float SIFT_DESCR_SCL_FCTR = 3.0, float SIFT_DESCR_MAG_THR = 0.2,
                            float SIFT_INT_DESCR_FCTR = 512.0, float _FLT_EPSILON = 1.19209290E-07) {
        int pt_c = int(std::roundf(c)), pt_r = int(std::roundf(r));
        float cos_t = std::cos(ori * (CV_PI / 180));
        float sin_t = std::sin(ori * (CV_PI / 180));
        ...
        for (int i = -radius; i < radius + 1; i++) {
            for (int j = -radius; j < radius + 1; j++) {
                ...
                if (rbin > -1 and rbin < d and cbin > -1 and cbin < d and rr > 0 and rr < rows - 1 and cc > 0 and
                    cc < cols - 1) {
    ...
    void extract(const cv::Mat &img, Keypoints &keypoints) {
        ...
        float k = powf(2, (1.f / n));
        float sigma[O][S];
        for (int i = 0; i < O; i++) {
            for (int j = 0; j < S; j++) {
                sigma[i][j] = powf(k, j) * sigma0 * (1 << i);
            }
        }
            ...
            for (int s = 1; s < S_ - 1; s++) {
                ...
                for (int i = 0; i < img.rows; i++) {
                    for (int j = 0; j < img.cols; j++) {
                        ...
                        int r_start = std::max(0, i - 1), r_end = std::min(i + 2, img.rows), c_start = std::max(0, j -
                                                                                                                   1), c_end = std::min(
                                j + 2, img.cols);
                        ...
                        if (std::abs(val) > threshold) {
                            ...
                            if (val > 0) {
                                for (int r = r_start; r < r_end; r++) {
                                    for (int c = c_start; c < c_end; c++) {
                                        if (i == r && j == c) {
                                            is_extreme = ((val >= img_prev.at<float>(r, c)) &&
                                                          (val >= img_next.at<float>(r, c)));
                                        } else {
                                            is_extreme = ((val >= img.at<float>(r, c)) &&
                                                          (val >= img_prev.at<float>(r, c)) &&
                                                          (val >= img_next.at<float>(r, c)));
                                        }
                                        if (!is_extreme) { break; }
                                    }
                            ...
                            if (is_extreme) {
                                ...
                                while (ii < SIFT_MAX_INTERP_STEPS) {
                                    ...
                                    dD = (cv::Mat_<float>(3, 1)
                                            << ((img.at<float>(x, y + 1) - img.at<float>(x, y - 1)) * deriv_scale),
                                    ...
                                    dxy = (img.at<float>(x + 1, y + 1) - img.at<float>(x + 1, y - 1) -
                                           img.at<float>(x - 1, y + 1) + img.at<float>(x - 1, y - 1)) *
                                          cross_deriv_scale;
                                    float dxs = (next.at<float>(x, y + 1) - next.at<float>(x, y - 1) -
                                                 prev.at<float>(x, y + 1) + prev.at<float>(x, y - 1)) *
                                                cross_deriv_scale;
                                    float dys = (next.at<float>(x + 1, y) - next.at<float>(x - 1, y) -
                                                 prev.at<float>(x + 1, y) + prev.at<float>(x - 1, y)) *
                                                cross_deriv_scale;
                                ...
                                if (!rejected) {
                                    ...
                                    float omax;
                                    std::vector<float> hist;
                                    GetMainDirection(GuassianPyramid[o][s_], x, y,
                                                     int(std::roundf(SIFT_ORI_RADIUS * scl_octv)),
                                                     SIFT_ORI_SIG_FCTR * scl_octv, BinNum, omax, hist);
//                                    std::cout << omax << std::endl;
                                    ...
                                    for (int k = 0; k < BinNum; k++) {
                                        ...
                                        if (hist[k] > hist[l] and hist[k] > hist[r2] and hist[k] >= mag_thr) {
                                            int bin =
                                                    k + 0.5 * (hist[l] - hist[r2]) / (hist[l] - 2 * hist[k] + hist[r2]);
                                            if (bin < 0) {
                                                bin = BinNum + bin;
                                            } else {
                                                if (bin >= BinNum) {
        ...
        for (int i = 0; i < keypoints.size(); i++) {
            ...
            calcSIFTDescriptor(GuassianPyramid[kpt.o][kpt.s], kpt.c * scale, kpt.r * scale, kpt.theta,
                               kpt.scale * scale * 0.5, d, n, kpt.descriptor);
        }
//        for(int i = 0;i < O;i++)
//        {
//            for(int j = 0;j < S-1;j++)
...
//        }
    ...
    void match(const Keypoints &src1, const Keypoints &src2, Keypoints &match1, Keypoints &match2, int max_pairs) {
        ...
        // also Shallow copy of _Data1
        cv::flann::GenericIndex<cvflann::L2<double> > kd(_Data1, cvflann::KDTreeIndexParams(4));
        ...
        // the Mat _Data1 should not be used after this, because it's a Shallow copy of the array.
        delete[] Data1;
        // the Mat _Data2 should not be used after this, because it's a Shallow copy of the array.
        delete[] Data2;
    ...
    cv::Mat linkPoints(const cv::Mat &img1, const Keypoints &match1, const cv::Mat &img2, const Keypoints &match2) {
        ...
        int i = 0;
        for (; i < match1.size(); i++) {
            cv::line(res, cv::Point(match1[i].c, match1[i].r), cv::Point(match2[i].c + img1.cols, match2[i].r),
                     cv::Scalar(0, 0, 255));
        }
        return res;
    }
